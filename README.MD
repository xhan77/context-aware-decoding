...
$$y_t \sim \text{softmax} \left[ \text{logit}_\theta \left( y_t \mid \boldsymbol{c}, \boldsymbol{x}, \boldsymbol{y}[<t] \right) \right]$$

Context-aware decoding samples tokens at each position with:
$$y_t \sim \text{softmax}[(1+\alpha) \text{logit}_\theta(y_t \mid \boldsymbol{c}, \boldsymbol{x}, \boldsymbol{y}_{\lt t}) - \alpha \text{logit}_\theta(y_t \mid \boldsymbol{x}, \boldsymbol{y}_{<t})]$$
